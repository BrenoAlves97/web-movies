...
export const CustomInput = ({ value, setValue, onClick, ...props }) => {
   return (
      <div className="w-full flex justify-end absolute -top-8 px-4 right-0">
         <div className="w-full max-w-xs flex gap-[.1rem] overflow-hidden rounded-sm">
            <input
               {...props}
               onChange={({ target }) => setValue(target.value)}
               value={value}
               className="w-full  p-2 outline-none bg-zinc-300 text-zinc-800 placeholder:text-zinc-600 hover:bg-zinc-100 focus:bg-white duration-200 text-base font-normal"
            />
            <button
